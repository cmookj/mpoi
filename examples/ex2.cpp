...
//                                Example 2
//
// This example reads an image file in ppm format, applies Gaussian blur both
// in sequential and parallel computation.
//
// ************************************************************************** //
#include "mpoi.h"
...
convolution_serial (const std::filesystem::path filepath, const bool save_gray = false) {
  ...
  if (save_gray) {
    write_pgm_binary (
        filepath.parent_path() / std::filesystem::path{filepath.stem().string() + "_gray.pgm"}, gray
    );
  }
