...
/* Create rotational factor */
__kernel void
spin_fact (__global float2* w, int n) {
...
/* Reverse bits */
__kernel void
bit_reverse (__global float2* dst, __global float2* src, int m, int n) {
...
/* Butterfly operation */
__kernel void
butterfly (__global float2* x, __global float2* w, int m, int n, int iter, uint flag) {
  unsigned int gid = get_global_id (0);
  unsigned int nid = get_global_id (1);

  int butterfly_size       = 1 << (iter - 1);
  int butterfly_grp_dist   = 1 << iter;
  int butterfly_grp_num    = n >> iter;
  int butterfly_grp_base   = (gid >> (iter - 1)) * (butterfly_grp_dist);
  int butterfly_grp_offset = gid & (butterfly_size - 1);

  int a = nid * n + butterfly_grp_base + butterfly_grp_offset;
  int b = a + butterfly_size;

  int l = butterfly_grp_num * butterfly_grp_offset;
...
/* Matrix transpose */
__kernel void
transpose (__global float2* dst, __global float2* src, int n) {
...
/* High-pass filter */
__kernel void
high_pass_filter (__global float2* image, int n, int radius) {
  unsigned int xgid = get_global_id (0);
  unsigned int ygid = get_global_id (1);
