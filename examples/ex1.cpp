...
//                                Example 1
//
// This example creates two arrays, a and b, and calculates a new array, c,
// where c[i] = f(a[i], b[i]).  The function, f, is defined below and in the
// kernel file.
//
// ************************************************************************** //
#include "mpoi.h"
...
main () {
  ...
  for (std::size_t i : std::views::iota (0, count_trials)) {
    ...
    // Calculate function f in sequential for verification.
    ...
    // Calculate RMS error between sequential and parallel calculations.
    float diff = 0.f;
    for (std::size_t j : std::views::iota (0, size)) {
      diff += pow (c[j] - d[j], 2);
    }
