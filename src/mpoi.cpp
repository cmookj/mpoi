  ...
  for (cl_uint i = 0; i != num_platforms; i++) {
    ...
    if (num_devices < 1) {
      ...
      for (cl_uint j = 0; j != num_devices; j++) {
        ...
        err = clGetDeviceInfo (
            deviceIDs[j], CL_DEVICE_MAX_COMPUTE_UNITS, sizeof (cl_uint), &max_compute_units, NULL
        );
...
mpoi::build_program (const std::string& src_file) {
  ...
  if (err != CL_SUCCESS) {
    ...
    clGetProgramBuildInfo (
        _program, _device_id, CL_PROGRAM_BUILD_STATUS, sizeof (cl_build_status), &build_status, NULL
    );

    if (build_status != CL_SUCCESS) {
      std::size_t ret_val_size;
      clGetProgramBuildInfo (_program, _device_id, CL_PROGRAM_BUILD_LOG, 0, NULL, &ret_val_size);
...
mpoi::set_kernel_argument (
  ...
  if (_kernels[kernel_id] != NULL) {
    cl_int err = clSetKernelArg (_kernels[kernel_id], static_cast<cl_uint> (order), size, mem);
...
mpoi::enqueue_data_parallel_kernel (
  ...
  if (_kernels[kernel_id] != NULL) {
    ...
    cl_int err = clEnqueueNDRangeKernel (
        _cmd_queue, _kernels[kernel_id], 1, NULL, &num_global_items, NULL, 0, NULL, NULL
    );
    if (err != CL_SUCCESS) {
...
mpoi::enqueue_data_parallel_kernel (
  ...
  if (_kernels[kernel_id] != NULL) {
    ...
    std::cout << "Local item size = " << num_local_items << std::endl;
    cl_int err = clEnqueueNDRangeKernel (
        _cmd_queue, _kernels[kernel_id], 2, NULL, global_size, NULL, 0, NULL, NULL
    );
    if (err != CL_SUCCESS) {
      std::cerr << "Error in enqueuing nd range kernel.\n";
      return;
